...
def log_tally_server_status(status):
    ...
    if 'expected_end_time' in status:
        ...
        if stopping_ts > time():
            expected_end_msg = ", collection ends in {}".format(format_delay_time_until(stopping_ts, 'at'))
        # we expect the collection to have stopped, and the TS should be
        # collecting results
        else:
            expected_end_msg = ", collection has ended, awaiting results for {}".format(format_elapsed_time_since(stopping_ts, 'since'))
