...
class TrafficModel(object):
    ...
    def increment_traffic_counters(self, strm_start_ts, byte_events, secure_counters):
        ...
        for i in xrange(num_packets):
            ...
            secure_counters.increment('ExitStreamTrafficModelEmissionCount', bin_value=SINGLE_BIN, num_increments=1)
            label = 'ExitStreamTrafficModelEmissionCount_{}_{}'.format(state, dir_code)
            secure_counters.increment(label, bin_value=SINGLE_BIN, num_increments=1)

            secure_counters.increment('ExitStreamTrafficModelLogDelayTime', bin_value=SINGLE_BIN, num_increments=ldelay)
            label = 'ExitStreamTrafficModelLogDelayTime_{}_{}'.format(state, dir_code)
            secure_counters.increment(label, bin_value=SINGLE_BIN, num_increments=ldelay)

            secure_counters.increment('ExitStreamTrafficModelSquaredLogDelayTime', bin_value=SINGLE_BIN, num_increments=ldelay*ldelay)
            label = 'ExitStreamTrafficModelSquaredLogDelayTime_{}_{}'.format(state, dir_code)
            secure_counters.increment(label, bin_value=SINGLE_BIN, num_increments=ldelay*ldelay)
            ...
            if (i+1) < num_states:
                ...
                secure_counters.increment('ExitStreamTrafficModelTransitionCount', bin_value=SINGLE_BIN, num_increments=1)
